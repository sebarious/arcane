				...
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						...
						ssrRenderList(typesForGame.value, (p) => {
							_push(`<option${ssrRenderAttr("value", p.type)}${ssrIncludeBooleanAttr(Array.isArray(unref(form).type) ? ssrLooseContain(unref(form).type, p.type) : ssrLooseEqual(unref(form).type, p.type)) ? " selected" : ""}${_scopeId}>${ssrInterpolate(p.type_label)} — ${ssrInterpolate(p.packs)} packs, £${ssrInterpolate(p.price_pounds.toFixed(2))}</option>`);
						});
						_push(`<!--]--></select></div></div>`);
						if (selectedProduct.value) _push(`<div class="bg-[#1a1628] border border-[rgba(124,58,237,0.25)] rounded-[8px] p-4 text-xs text-[#a3a3a3] font-[&#39;Jost&#39;,sans-serif]"${_scopeId}><strong class="text-white"${_scopeId}>${ssrInterpolate(selectedProduct.value.type_label)}</strong> — ${ssrInterpolate(selectedProduct.value.packs)} sealed mystery packs, invoiced at <strong class="text-[#c9a84c]"${_scopeId}>£${ssrInterpolate(selectedProduct.value.price_pounds.toFixed(2))}</strong> ex VAT, due 48 hours after generation. </div>`);
						else _push(`<!---->`);
						...
						createVNode("div", { class: "grid grid-cols-2 gap-4" }, [createVNode("div", null, [createVNode("label", { class: "block font-['Jost',sans-serif] font-semibold text-xs uppercase tracking-wide text-[rgba(255,255,255,0.35)] mb-2" }, "Game"), withDirectives(createVNode("select", {
							...
							return openBlock(), createBlock("option", {
								key: p.type,
								value: p.type
							}, toDisplayString(p.type_label) + " — " + toDisplayString(p.packs) + " packs, £" + toDisplayString(p.price_pounds.toFixed(2)), 9, ["value"]);
						...
						selectedProduct.value ? (openBlock(), createBlock("div", {
							...
							createVNode("strong", { class: "text-[#c9a84c]" }, "£" + toDisplayString(selectedProduct.value.price_pounds.toFixed(2)), 1),
							createTextVNode(" ex VAT, due 48 hours after generation. ")
...
//# sourceMappingURL=BatchRequest-Bn9NdDz9.js.map